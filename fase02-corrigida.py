...
def decadaMaisChuvosa():
    ...
    plt.title("Precipitação (mm/m²) média por década") # título do gráfico
...
def exibeDadosAgostos():
    ...
    for ano in range(2006,2016): # exibe até 2015
        temperaturas.append(dadosTempAgosto(ano))
        umidades.append(dadosUmidadeAgosto(ano))
        ventos.append(dadosVentoAgosto(ano))
        print("\n")
    print("Sem dados para o mês de agosto de 2016")
    print('\nConsiderando os dados de todo o período...')
    tempmedia = ventomedio = umidmedia = 0
    totaltemp = totalvento = totalumid = 0
    modastemp = []
    modasumid = []
    modasvento = []
    ...
    tempmedia = tempmedia/totaltemp
    tempmoda = statistics.mode(modastemp)
    ventomedio = ventomedio/totalvento
    ventomoda = statistics.mode(modasvento)
    umidmedia = umidmedia/totalumid
    umidmoda = statistics.mode(modasumid)
    print(f'\nTemperatura do período: Média = {tempmedia}; Moda = {tempmoda}')
    print(f'\nUmidade do período: Média = {umidmedia}; Moda = {umidmoda}')
    print(f'\nVento do período: Média = {ventomedio}; Moda = {ventomoda}')
